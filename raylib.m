...
:- pred begin_shader_mode(rshader::in, io::di, io::uo) is det.
:- pred end_shader_mode(io::di, io::uo) is det.
:- pred is_shader_ready(rshader::in, bool::out, io::di, io::uo) is det.
...
:- pred unload_shader(rshader::in, io::di, io::uo) is det.
...
:- pragma foreign_type("C", rshader,  "Shader*", [can_pass_as_mercury_type]).
% :- pragma foreign_type("C", rcolor,  "Color", [can_pass_as_mercury_type]).
...
:- pragma foreign_proc(
    "C", unload_shader(RShader::in, _IO0::di, _IO::uo),
    [ promise_pure, will_not_call_mercury, will_not_throw_exception
    , will_not_modify_trail, thread_safe, does_not_affect_liveness
    , tabled_for_io],
    "
        Shader* p = (Shader*)RShader;
        UnloadShader(*p);
        MR_GC_free((void*)RShader);
    ").

:- pred load_shader_(string::in, string::in, rshader::out, io::di, io::uo) is det.

:- pragma foreign_proc(
    "C", load_shader_(VS::in, FS::in, Shader0::out, _IO0::di, _IO::uo),
    [ promise_pure, will_not_call_mercury, will_not_throw_exception
    , will_not_modify_trail, thread_safe, does_not_affect_liveness
    , tabled_for_io],
    "
        ...
        Shader0 = MR_GC_malloc_uncollectable(sizeof(Shader));
        Shader shader = LoadShader(vsFilename, fsFilename);
        printf(""**** shader loaded: %u\\n"", shader.id);
        memcpy(Shader0, &shader, sizeof(Shader));
    ").

:- pragma foreign_proc(
    "C", is_shader_ready(RShader::in, Ready::out, _IO0::di, _IO::uo),
    [ promise_pure, will_not_call_mercury, will_not_throw_exception
    , will_not_modify_trail, thread_safe, does_not_affect_liveness
    , tabled_for_io],
    "
        Ready = IsShaderReady(*(Shader*)RShader) ? MR_YES : MR_NO ;
    ").

:- pragma foreign_proc(
    "C", begin_shader_mode(RShader::in, _IO0::di, _IO::uo),
    [ promise_pure, will_not_call_mercury, will_not_throw_exception
    , will_not_modify_trail, thread_safe, does_not_affect_liveness
    , tabled_for_io],
    "
...
:- pragma foreign_proc(
    "C", end_shader_mode(_IO0::di, _IO::uo),
    [ promise_pure, will_not_call_mercury, will_not_throw_exception
    , will_not_modify_trail, thread_safe, does_not_affect_liveness
    , tabled_for_io],
    "
...
:- pragma foreign_proc(
    ...
    , tabled_for_io],
    "
        DrawRectangleRounded((Rectangle){ X, Y, W, H }, Roundness, Segments, *((Color*)&C));
    ").
...
:- pragma foreign_proc(
    ...
    , tabled_for_io],
    "
       DrawCircleV((Vector2){ X, Y }, Radius, *((Color*)&Clr));
    ").
...
:- pragma foreign_proc(
    ...
    , tabled_for_io],
    "
        DrawCircleGradient((int)Cx, (int)Cy, Radius, *((Color*)&Color1), *((Color*)&Color2));
    ").
...
:- pragma foreign_proc(
    ...
    "
        SUCCESS_INDICATOR = CheckCollisionPointCircle(
            (Vector2){Px, Py}, (Vector2){Cx, Cy}, Radius);
    ").
...
:- pragma foreign_proc(
    ...
    "
        SUCCESS_INDICATOR = CheckCollisionRecs(
            (Rectangle){RX1, RY1, RW1, RH1}, (Rectangle){RX2, RY2, RW2, RH2});
    ").
...
:- pragma foreign_proc(
    ...
    "
        SUCCESS_INDICATOR = CheckCollisionPointRec(
            (Vector2){PX, PY}, (Rectangle){RX2, RY2, RW2, RH2});
    ").
...
:- pragma foreign_proc(
    ...
    , tabled_for_io],
    "
        Out = MR_GC_malloc_uncollectable(sizeof(Texture2D));
        Texture2D tex = LoadTexture(Filename);
        memcpy(Out, &tex, sizeof(Texture2D));
    ").
...
:- pragma foreign_proc(
    ...
    , tabled_for_io],
    "
        Texture2D* pTex  = (Texture2D*)RTexture;
        Rectangle source = { SrcX, SrcY, SrcW, SrcH };
        Rectangle dest   = { DstX, DstY, DstW, DstH };
        Vector2   origin = { X, Y };
        Color tint;
        memcpy((void*)&tint, &Colour, sizeof(Color));
        DrawTexturePro(*pTex, source, dest, origin, Rotation, tint);
    ").
